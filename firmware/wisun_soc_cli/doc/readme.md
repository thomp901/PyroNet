...
### Matching a `wsbrd` Border Router

For a border router configured with:
...
the default client settings in this project already match those values through `config/wisun/wisun_settings.wisunconf`.

The equivalent runtime CLI commands are:

    wisun set wisun.network_name "Wi-SUN Network"
    wisun set wisun.regulatory_domain NA
    wisun set wisun.chan_plan_id 1
    wisun set wisun.phy_mode_id 2
    wisun save

This project also stores the client credential material in `config/wisun/wisun_settings.wisunconf`:

- `devicePrivateKey`: node private key
- `deviceCertificate`: node certificate
- `caCertificate`: trusted CA certificate

These must match the border router CA, while the node certificate and key must remain distinct from the border router certificate and key.
