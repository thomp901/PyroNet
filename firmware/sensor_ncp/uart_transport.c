...
#define UART_LOG_BUFFER_SIZE 128U
#define SENSOR_NCP_VERSION "sensor_ncp uart-bootstrap v0.1"

static void logLine(UartTransportLogFn logFn, const char *line)
{
    if (logFn != NULL)
    {
        logFn(line);
...
static void processCommand(UART2_Handle uartHandle,
                           const char *command,
                           UartTransportLogFn logFn)
{
    char lineBuffer[UART_LOG_BUFFER_SIZE];

    snprintf(lineBuffer, sizeof(lineBuffer), "UART_CMD: %s", command);
    logLine(logFn, lineBuffer);
...
void uartTransportRun(UartTransportLogFn logFn)
{
    ...
    if (uartHandle == NULL)
    {
        logLine(logFn, "UART_INIT: failed");
        while (1) {}
    }

    snprintf(lineBuffer,
             sizeof(lineBuffer),
             "UART_INIT: ready baud=%lu tx_dio=%u rx_dio=%u",
             (unsigned long)UART_BAUD_RATE,
             UART_TX_DIO,
             UART_RX_DIO);
    logLine(logFn, lineBuffer);
    ...
    while (1)
    {
        ...
        if (rxByte == '\n')
        {
            ...
            processCommand(uartHandle, commandBuffer, logFn);
            commandLength = 0;
            continue;
        }
